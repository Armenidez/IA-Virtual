...
def on_click(event):
    ...
    if can_drag:
        ...
        if Flag_Ventana_opciones:
            ...
            print("1")
            '''if Flag_Text_Box:
            dx3, dy3 = event.x_root-ventana_BT.winfo_rootx(), event.y_root-ventana_BT.winfo_rooty()
            print("2")'''
...
def Imagen_Asistente(Gesto):
    ...
    Im1 = Im1.resize((weight,int(Height)), PIL.Image.ANTIALIAS)
...
def Abrir_Ventana_Opciones(event):
    ...
    ventana_opciones_cerrar.bind('<Button-1>', lambda event: main.destroy())
    ...
    ventana_opciones_cerrar_chiquito.bind('<Button-1>', lambda event:Insertar_texto("Hola soy tu asistente virtual y estoy aqui para ayudarte en todo ☺"))
